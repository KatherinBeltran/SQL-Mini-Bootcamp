-- ENTREGA WEEK 5 — TECHMART DASHBOARD
-- SQL & MySQL Bootcamp
...
-- R9: Análisis de descuentos (rangos con CASE WHEN)
SELECT
    CASE
...
SELECT
    ...
    COUNT(v.id) AS num_compras,
    CONCAT('$', FORMAT(SUM(v.cantidad * v.precio_unitario * (1 - v.descuento/100)), 2)) AS gastado,
    DATEDIFF(MAX(v.fecha_venta), MIN(v.fecha_venta)) AS dias_como_cliente_activo
FROM clientes cl
JOIN ventas v ON cl.id = v.cliente_id
GROUP BY cl.id, cl.nombre, cl.email
HAVING COUNT(v.id) >= 2
ORDER BY num_compras DESC, gastado DESC;
...
SELECT
    ...
    ROUND(
        100.0 *
        SUM(v.cantidad * v.precio_unitario * (1 - v.descuento/100))
        / (SELECT SUM(cantidad * precio_unitario * (1 - descuento/100)) FROM ventas),
        2
    ) AS pct_del_total
FROM categorias c
JOIN productos p ON c.id = p.categoria_id
...
SELECT
    ...
    COALESCE(SUM(v.cantidad), 0) AS unidades_vendidas,
    ROUND(
        100.0 * COALESCE(SUM(v.cantidad), 0) / NULLIF(SUM(p.stock), 0),
        2
...
-- R13: Crecimiento mensual (self-join, sin window functions)
SELECT
    actual.periodo,
    CONCAT('$', FORMAT(actual.revenue, 2))   AS revenue,
    CONCAT('$', FORMAT(anterior.revenue, 2)) AS revenue_mes_anterior,
    ROUND(
        100.0 * (actual.revenue - anterior.revenue) / NULLIF(anterior.revenue, 0),
        2
    ) AS crecimiento_pct
FROM (
    SELECT
...
-- PARTE 3: BONUS — 2 REPORTES ADICIONALES

-- BONUS 1: ¿Qué día de la semana se vende más?
SELECT
...
FROM ventas
GROUP BY dia_semana
ORDER BY SUM(cantidad * precio_unitario * (1 - descuento/100)) DESC;

-- BONUS 2: Productos que nunca se han vendido
SELECT
    p.nombre AS producto,
    c.nombre AS categoria,
    p.stock,
